...
import sk.tuke.fei.kpi.nosal.milan.bte.metaconfiguration.CopyBranchType;

/**
 * Procesor pre spracovanie metainformacie CopyBranch.
 * @author Milan
 */
public class CopyBranchProcessor implements Processor {

    public ConfigurationType process(ConfigurationType configuration) {
        // Spracujem CopyBranch metainformacie
        processCopyBranch(configuration, configuration);
        return configuration;
    }

    /**
     * Vlastna metoda pre spracovanie metainformacie CopyBranch. Preberam dva
     ...
     * @param model
     */
    private void processCopyBranch(ConfigurationType configuration, ConfigurationType model) {
        // Najdem metainformaciu
        CopyBranchType metaConfiguration = getCopyBranch(configuration);
        // inside je polozka, ktora sa stane potomkom v novom vztahu
        ConfigurationType copy;
        // Spracuvam metainformaciu ak tu je
        if(metaConfiguration!=null && !configuration.isCloned()){
            ...
            // nezmyselne
            copy = configuration.cloneBranch();
            // Pokusime sa najst rodica
            ConfigurationType newParent = findConfiguration(model, metaConfiguration.getCopysParent());
            if(newParent!=null) {
                ...
                if(newParent.getMappingOfConfigurationToXML().getXMLOutputType()==XMLProcessing.ATTRIBUTE){
                    throw new RuntimeException("BTE.CopyBranchProcessor::\n\tERROR: Misuse of CopyBranch metaconfiguration.");
                }
                // Podla typu zdroja informacii k polozke
                switch(copy.getMappingOfConfigurationToSources().getSourceType()){
                    case ANNOTATION:{
                        // Ak ide o anotacny typ
                        copy.setParent(newParent);
                        newParent.getChildren().add(copy);
                        // Tak proste ako kotvu uvedieme noveho rodica
                        copy.getMappingOfConfigurationToSources().setPositionAnchor(newParent);
                        copy.getMappingOfConfigurationToSources().setRelPositionToAnchor(RelativePositionToAnchor.SAME_LOWER_LVL);
                        break;
                    }
                    ...
                    case DECL_PROP_STRING:{
                        // Ak je vsak kopia vlastnostou, je
                        ...
                        copy.setParent(configuration.getParent());
                        ConfigurationType substitute = getSubstitute(copy);
                        if(substitute!=null){
                            ...
                            copy.getMappingOfConfigurationToXML().setMinOccurs(0);
                            copy.getMappingOfConfigurationToXML().setMaxOccurs(-1);
                            ...
                            substitute.getMappingOfConfigurationToSources().setRelPositionToAnchor(RelativePositionToAnchor.SAME_LOWER_LVL);
     ...
     * ostane iba cesta ku kopirovanej polozke, teda ku configuration.
     * @param configuration
     * @return
     */
    private ConfigurationType getSubstitute(ConfigurationType configuration){
        // Pripravim si pomocne premenne
        // Ret vetvu, ktora sa ma kopirovat v hierarchii
        ...
        while(configuration!=null){
            ...
            if(configuration.equals(temp)){
                switch(configuration.getMappingOfConfigurationToSources().getSourceType()){
                    ...
                    default:{
                        throw new ParsingException("CopyBranchProcessor::\n ERROR:\t"
    ...
    /**
     * Metoda vrati metainformaciu a odstrani ju zo zoznamu.
     * @param configuration
     * @return
     */
    private CopyBranchType getCopyBranch(ConfigurationType configuration){
        for(Object metaConfiguration : configuration.getMetainformations()){
            if(metaConfiguration instanceof CopyBranchType){
                configuration.getMetainformations().remove(metaConfiguration);
                return (CopyBranchType)metaConfiguration;
            }
        }
        return null;
    }
